...
def save_hospitals_to_json(hospitals, filename):
    data = []
...
def get_nearest_hospitals(zipcode, api_key):
    gmaps = googlemaps.Client(key=api_key)
    try:
        # Geocode the ZIP code to get its coordinates
        ...
        if geocode_result:
            ...
            # Use the coordinates to search for nearby hospitals
            places = gmaps.places_nearby(location=(latitude, longitude), radius=10000, keyword='hospital')
            return places['results']
    except googlemaps.exceptions.ApiError as e:
        logger.error(f"Google Maps API Error: {e}")
        return None

def show_nearest_hospitals(request):
    api_key = ""  # Replace with your API key
    zipcode = request.GET.get('zipcode')
    if not zipcode:
        return JsonResponse({'error': 'No ZIP code provided'}, status=400)

    hospitals = get_nearest_hospitals(zipcode, api_key)
...
def MakeMent(request):
    ...
    logger.debug("zip6"+str(zipcode))
    hospitals = get_nearest_hospitals(str(zipcode), "")  # You need to implement this function
    logger.debug("hospi5")
    logger.debug(hospitals)
	 # Convert hospitals object to JSON
    hospitals_json = json.dumps(hospitals)

    # Pass JSON data as context to maps.html template
    return render(request, 'maps.html', {'hospitals_json': hospitals_json})
